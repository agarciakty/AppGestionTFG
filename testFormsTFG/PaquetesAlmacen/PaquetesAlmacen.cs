...
namespace testFormsTFG.PaquetesAlmacen
{
    public partial class PaquetesAlmacen : Form
    {
        ...
        DataTable datosparcelas = new DataTable();
        DataTable historial = new DataTable();

        FuncionesBD fbd = new FuncionesBD();
        public PaquetesAlmacen()
        {
            InitializeComponent();
        }

        private void PaquetesAlmacen_Load(object sender, EventArgs e)
        {
            this.panelHistorial.Visible = false;
            this.panelPopup.Visible = false;
            this.panelCorrecta.Visible = false;
            table = fbd.obtenerAlmacenes();

            string almacen = "";

            foreach (DataRow row in table.Rows)
            {
                almacen = Convert.ToString(row["NOMBRE"]);
                almacen += " #" + Convert.ToString(row["COD_ALMACEN"]);
        ...
        private void cargarCombos()
        {
            ...
            datosparcelas = fbd.obtenerDatosParcelas(codalm);
            ...
            {
                if (!naves.Contains(row["NAVE"].ToString()))
                {
                    naves.Add(row["NAVE"].ToString());
            ...
            /*if (Convert.ToInt32(datosparcelas.Rows[0][3]) > 0)
            {
                ...
                tableFuera = fbd.obtenerDatosFuera(codalm);
            }*/
        ...
        private void comboNave_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            foreach (DataRow row in datosparcelas.Rows)
            {
                if (!secciones.Contains(row["FILA"].ToString()) && row["NAVE"].ToString() == this.parcela[0])
                {
                    secciones.Add(row["FILA"].ToString());
                    comboFila.Items.Add(row["FILA"].ToString());
                }

                if (!columnas.Contains(row["COLUMNA"].ToString()) && row["NAVE"].ToString() == this.parcela[0])
                {
                    columnas.Add(row["COLUMNA"].ToString());
                    comboCol.Items.Add(row["COLUMNA"].ToString());
                }
            }
        ...
        private void comboFila_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            foreach (DataRow row in datosparcelas.Rows)
            {
                if (!columnas.Contains(row["COLUMNA"].ToString()) && row["NAVE"].ToString() == this.parcela[0] && row["FILA"].ToString() == this.parcela[1])
                {
                    columnas.Add(row["COLUMNA"].ToString());
                    comboCol.Items.Add(row["COLUMNA"].ToString());
                }
        ...
        private void btnGuardar_Click(object sender, EventArgs e)
        {
            ...
            if (fbd.checkMismaParcela(this.textBox2.Text, parcelaComp, this.codalm))
            {
                this.panelPopup.Visible = true;
                this.btnPopup.Visible = true;
                this.panelPopup.BackColor = Color.Silver;
            ...
            fbd.guardarParcela(this.textBox2.Text, this.codalm, parcela);
        ...
        {
            this.historial = fbd.historialPaquete(this.codalm, this.textBox1.Text);
